...
Raven Python Worker 在同步成功后按 `EDGE_SNAPSHOT_RETENTION_VERSIONS` 自动清理旧快照归档，默认保留 2 个版本。也可在独立 [Raven 仓库](https://github.com/superbinf/raven) 中执行 `make snapshots-cleanup` 预览清单；清理不会触碰当前业务数据库中的投影数据。

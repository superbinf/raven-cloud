...
export function CredentialManagementPage({ tenantId }: { tenantId?: string }) {
  ...
  return <><PageHeader eyebrow="DEPLOYMENT CREDENTIALS" title="部署凭据" description="按当前客户的地端实例管理许可证与 OpenAPI Key，不在此处维护账号泄露数据。" actions={<Button variant="secondary" onClick={() => void refresh()} disabled={state.loading}><RefreshCw size={16} />刷新状态</Button>} />
